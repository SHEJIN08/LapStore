  ...
  try {
    ...
    if (!categoryName) {
      return res
        ...
        .json({ success: false, message: "Please fill category name" });
    }
    ...
    return res
      ...
      .json({ success: true, message: ResponseMessage.CREATED });
  } catch (err) {
    if (err.message === ResponseMessage.DUP_CATEGORY) {
      return res
        .status(StatusCode.BAD_REQUEST)
        .json({ success: false, message: ResponseMessage.DUP_CATEGORY });
    }
    console.error(err);
    return res
      .status(StatusCode.INTERNAL_SERVER_ERROR)
...
const getEditCategory = async (req, res) => {
  try {
    ...
    const { category, products } =
      await categoryService.getCategoryWithProductsService(id);
...
const editCategory = async (req, res) => {
  try {
    ...
    await categoryService.updateCategoryService(id, {
      categoryName,
      description,
      isListed,
    });
    ...
    return res
      ...
      .json({ success: true, message: ResponseMessage.CATEGORY_STATUS });
  } catch (error) {
...
const getListOrUnlist = async (req, res) => {
  try {
    ...
    return res
      .status(StatusCode.OK)
      .json({ success: true, message: `Category ${statusText} successfully` });
  } catch (error) {
    if (error.message === "Category not found") {
      return res.status(StatusCode.NOT_FOUND).render("user/404");
    }
