...
const getBrandPage = async (req, res) => {
  try {
    ...
    const { brands, totalBrands, totalPages } =
  ...
  try {
    ...
    if (!req.file) {
      return res
        ...
        .json({ success: false, message: ResponseMessage.BRAND });
    }
    ...
    return res
      ...
      .json({ success: true, message: ResponseMessage.NEW_BRAND });
  } catch (err) {
    if (err.message === ResponseMessage.DUP_BRAND) {
      return res
        .status(StatusCode.BAD_REQUEST)
        .json({ success: false, message: ResponseMessage.DUP_BRAND });
    }
    console.error(err);
    return res
...
const BlockOrUnblock = async (req, res) => {
  try {
    ...
    return res
      .status(StatusCode.OK)
      .json({ success: true, message: ResponseMessage.BRAND_STATUS });
  } catch (err) {
    if (err.message === ResponseMessage.BRAND_NOT_FOUND) {
      return res
        .status(StatusCode.BAD_REQUEST)
        .json({ success: false, message: ResponseMessage.BRAND_NOT_FOUND });
    }
    console.error(err);
    return res
      .status(StatusCode.INTERNAL_SERVER_ERROR)
