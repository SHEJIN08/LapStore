...
const loadAddProduct = async (req, res) => {
  try {
    const { categories, brands } =
      await productService.getAddProductDataService();
...
const BlockOrUnblock = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({ success: true, message: ResponseMessage.PRODUCT_STATUS });
  } catch (err) {
    if (
      err.message === ResponseMessage.BAD_REQUEST ||
      err.message === ResponseMessage.PRODUCT_NOT_FOUND
    ) {
      return res
...
const loadEditProduct = async (req, res) => {
  try {
    ...
    const { product, variants, categories, brands } =
      await productService.getEditProductDataService(id);
    ...
    if (err.message === ResponseMessage.PRODUCT_NOT_FOUND) {
      return res
        ...
        .json({ success: false, message: ResponseMessage.PRODUCT_NOT_FOUND });
    }
...
const uploadVariantImage = async (req, res) => {
  try {
    ...
    if (!file) {
      return res
        .status(StatusCode.BAD_REQUEST)
        .json({ success: false, message: ResponseMessage.VAR_PIC });
    }

    const image = await productService.updateVariantImageService(
      variantId,
      file.secure_url
    );
...
const editProduct = async (req, res) => {
  try {
    ...
    if (error.message === ResponseMessage.PRODUCT_NOT_FOUND) {
      return res
        .status(StatusCode.NOT_FOUND)
        .json({ success: false, message: ResponseMessage.VARIANT_NOT_FOUND });
    }
