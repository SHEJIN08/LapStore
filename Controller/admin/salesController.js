  ...
  try {
    ...
    if (format === "pdf") {
      ...
      orders.forEach((order) => {
        ...
        const productName =
          order.deliveredItems && order.deliveredItems[0]
            ? order.deliveredItems[0].productName
            : "Unknown Item";

        doc
          .fontSize(9)
          .text(productName, itemX, y, { width: 200, ellipsis: true });
        doc
          .fontSize(8)
          .text(`#${order.orderId}`, itemX, y + 12, { color: "grey" }); // Subtext
