...
const addAddress = async (req, res) => {
    try {
        ...
        if(!addressType) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please select a address type'});
        if(!['Home', 'Work', 'Other'].includes(addressType)) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please choose address from the given option'});
        if(!name) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please enter a name'});
        if(phone.length < 10) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please enter a valid number'});
        if(!addressLine1) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please enter a address'});
        if(!city) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please choose a city'});
        if(!state) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please choose a state'});
        if(pincode.length !== 6) return res.status(StatusCode.BAD_REQUEST).json({success: false, message: 'Please enter a valid pincode'});

    
        await addressService.addAddressService(userId, {
            addressType, name, phone, addressLine1, addressLine2, city, state, pincode
        });
