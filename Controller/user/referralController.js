...
const loadReferralPage = async (req, res) => {
  try {
    ...
    const { user, referrals, totalEarnings, successfulCount } =
      await referralService.userReferrerData(userId);
    ...
    res.render("user/referral", {
      ...
      totalEarnings,
      successfulCount,
      referralLink: `${req.protocol}://${req.get("host")}/user/register?ref=${
        user.referralCode
      }`,
    });
  } catch (error) {
