...
const loadWishlist = async (req,res) => {
    try{
       const userId = req.session.user;
        ...
        const { items, totalPages, currentPage } = await wishlistService.getWishlist(userId, page, limit);
...
const addToWishlist = async (req,res) => {
    try {
        ...
        if(!userId){
          return  res.status(StatusCode.UNAUTHORIZED).json({success: false, message: 'Please login to add items'})
        }

        await wishlistService.addToWishlist(userId, productId, variantId)
        
        res.status(StatusCode.OK).json({success: true, message: 'Item added to wishlist'})
    } catch (error) {
        console.log(error)
...
const removeFromWishlist = async (req,res) => {
    try {
        ...
        res.status(StatusCode.OK).json({success: true, message: 'Item removed from the wishlist'})
