...
const loadHome = async (req, res) => {
  try {
    ...
    const { categories, brands, products } =
      await userService.getHomeDataService();
...
const detailedPage = async (req, res) => {
  try {
    ...
    const reviews = await reviewService.getProductReviewService(
      data.product._id
    );
    const totalReviews = await reviewService.totalReviewCountService(
      data.product._id
    );
...
const shopPage = async (req, res) => {
  try {
    ...
    const { products, totalPages, categories, brands } =
      await userService.getShopProductsService({
        page,
        limit,
        search,
        sortOption,
