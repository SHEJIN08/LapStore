...
// --- CREATE CATEGORY ---
const createCategoryService = async ({ categoryName, description, isListed, count }) => {
    // Check Duplicate
...
const getCategoryWithProductsService = async (categoryId) => {
    ...
    // Fetch Products
    const productDocs = await Product.find({ category: categoryId }).sort({ createdAt: -1 });
...
// --- UPDATE CATEGORY ---
const updateCategoryService = async (id, { categoryName, description, isListed }) => {
