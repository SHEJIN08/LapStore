...
const getOfferService = async ({ search, status, page, limit }) => {
  try {
    ...
    if (search) {
      ...
      query.$or = [{ offerName: searchRegex }];
    }
    ...
    if (discountType === "fixed") {
      ...
      if (offerType === "product") {
        ...
        const invalidVariant = variants.find(
          (v) => fixedVal > v.salePrice * 0.8
        );

        if (invalidVariant) {
          const maxAllowed = Math.floor(invalidVariant.salePrice * 0.8);
          throw new Error(
            `Fixed discount (₹${fixedVal}) exceeds 80% of the price for ${
              invalidVariant.variantName || "a product variant"
            }. 
        ...
        if (!productsInCategory.length) {
          throw new Error(
            "This category has no products to apply an offer to."
          );
        }

        const categoryProductsId = productsInCategory.map((s) => s._id);

        const categoryVariants = await Variant.find({
          productId: { $in: categoryProductsId },
        });
        const invalidVariant = categoryVariants.find(
          (v) => fixedVal > v.salePrice * 0.8
        );
        if (invalidVariant) {
          const maxAllowed = Math.floor(invalidVariant.salePrice * 0.8);
