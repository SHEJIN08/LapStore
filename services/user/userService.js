...
const getProductDetailsService = async (slug) => {
  ...
  if (product.offerId) {
    const offerDoc = await Offer.findById(product.offerId).select(
  ...
  const relatedProducts = await Promise.all(
    relatedProductsRaw.map(async (rel) => {
      ...
      if (offerData.offerId) {
        const offerDoc = await Offer.findById(offerData.offerId).select(
          "discountType discountValue"
        );
        if (offerDoc) {
  ...
  const products = await Promise.all(
    productsRaw.map(async (product) => {
      ...
      const { finalPrice, discountAmount, offerId } =
        await calculateProductDiscount(productForCalc);
      ...
      if (offerId) {
        const offerDoc = await Offer.findById(offerId).select(
          "offerType discountValue discountType"
        );
        if (offerDoc) {
          product.offerType = offerDoc.discountType;
          product.offerValue = offerDoc.discountValue;
