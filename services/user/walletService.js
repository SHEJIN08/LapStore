...
const verifyAndCreditWallet = async (userId, razorpay_order_id, razorpay_payment_id, razorpay_signature) => {
...
const processWalletPayment = async (userId, amount, orderId) => {
    try {
        ...
        const transaction = new walletTransactions({
            ...
            description: `Payment for Order #${orderId.toString().slice(-6).toUpperCase()}`
        });
...
export default {getWalletData, initiateAddMoney, verifyAndCreditWallet, processWalletPayment}