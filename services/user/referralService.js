...
const checkAndCreditReferral = async (userId) => {
    try {
        const referral = await Referral.findOne({refereeId: userId, status: 'Pending'})
        if(!referral){
            return;
        }
        ...
        if(!referralWallet){
             referralWallet =  new Wallet({userId: referral.referrerId, balance: referral.referralAmount})
        ...
        console.log(`Referral reward of ${referral.referralAmount} credited to ${referral.referrerId}`);
        return true;
    } catch (error) {
