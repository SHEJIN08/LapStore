  ...
  const processedCartItems = await Promise.all(
    cartItems.map(async (item) => {
      ...
      const { finalPrice, discountAmount, offerId } =
        await calculateProductDiscount(productForOffer);
...
const placeOrderService = async (
  ...
  for (const item of cartItems) {
    ...
    const { finalPrice, offerId } = await calculateProductDiscount(
      productForOffer
    );
    ...
    orderedItems.push({
      ...
      image: Array.isArray(item.variantId.image)
        ? item.variantId.image[0]
...
const saveFailedOrderService = async (
  userId,
  addressId,
  paymentDetails,
  couponCode = null
...
const getOrderDetails = async (id) => {
  try {
    let order = await Order.findOne({ orderId: id }).populate(
